...
Every interface image in this README is generated by the repository's documentation renderer with fixed demonstration data. The quota and refresh images render the production `CompactStatusItemView`, the settings image uses real editor pages, and the three-theme overview uses the source theme preview components with the same appearance resolver. The renderer adds explanations only around those views; the quota, countdown, and refresh states shown are not the reader's real data.

The screenshots retain the current production UI labels. This English README translates the documentation only and does not claim that the application interface itself is localized into English.
...
- Automatically adapts between dual-window and single-window quota layouts
- Refreshes at launch, manually, on schedule, and after wake from sleep
- Confirms short-lived failures, keeps the latest reliable value, and retries recovery automatically
- Uses freshness-limited local fallback data when the app-server is unavailable
- Sends a local notification when quota is low
- Offers optional voice announcements at 1-, 5-, or 10-minute intervals
- Provides LOUD, BOLD, and FROST as independently persisted themes
- Separates panel, menu bar status-item, and state-color settings
- Uses the native macOS color panel with opacity and continuous preview
- Collects no telemetry and uploads no prompts, replies, attachments, or quota records
...
- **LOUD**, **BOLD**, and **FROST** each have an independent color configuration.
- Panel typography, stroke, corner radius, shadow, and surface opacity are saved per theme.
- Menu bar font, stroke, corner radius, shadow, padding, and height are also saved per theme.
- The settings page's own font scale is global and does not change with the selected theme.
...
| Base palette | Background, surface, text and stroke, action controls |
