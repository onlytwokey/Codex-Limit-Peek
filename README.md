...
- **状态栏显示层：** 主额度与周额度文字颜色、阴影颜色与透明度、水平/垂直偏移，以及独立的字体、描边、圆角、横向留白和高度。
...
| 状态栏显示层 | 主额度文字色、周额度文字色、阴影色、阴影透明度 0%–100%、水平/垂直偏移 -10–10 pt、模糊 0–8 pt、字体 8–14 pt、描边 0–4 pt、圆角 0–12 pt、横向留白 2–14 pt、高度 14–22 pt |
...
每个颜色项都提供预设色块，也可以通过方形“＋”按钮打开 macOS 原生取色面板。基础色板与状态颜色支持透明度；状态栏主额度、周额度和阴影颜色按不透明颜色保存。连续调整会实时进入当前预览，并保存到打开取色器时选中的主题。
